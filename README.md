...
Use `scripts/annotations/agreement_scores.py` to export disagreement plus phase-appropriate IAA outputs for annotation datasets:
...
- `<stem>_krippendorff_summary.csv` with per-label Krippendorff's alpha computed from items that have at least 3 annotators by default
- `<stem>_cohen_kappa_scores.csv` with pairwise Cohen's kappa by label and annotator pair, computed from items that have exactly 2 annotators by default
- `<stem>_cohen_kappa_summary.csv` with per-label aggregate Cohen's kappa stats for the 2-annotator phase
- `<stem>_videos_used_for_kappa.csv` listing the item/video rows used in each kappa calculation

This matches the common workflow where Krippendorff's alpha is reported for a 3-annotator calibration phase and Cohen's kappa is reported for a later 2-annotator phase. You can override the defaults with `--krippendorff_min_annotators` and `--cohen_exact_annotators`.
